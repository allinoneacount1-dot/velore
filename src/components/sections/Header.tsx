...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[var(--bg)] shadow-[0_4px_12px_rgb(163,177,198,0.15)]">
      <div className="mx-auto flex h-18 max-w-7xl items-center px-6 lg:px-12">
        {/* Logo */}
        ...
        <nav className="ml-auto hidden items-center gap-10 md:ml-0 md:flex md:flex-1 md:justify-center" aria-label="Main navigation">
          {NAV_LINKS.map((link) => (
            <a
              key={link.label}
              href={link.href}
        ...
        {/* Mobile Hamburger */}
        <button
          className="ml-auto flex h-11 w-11 items-center justify-center rounded-xl bg-[var(--bg)] shadow-[var(--neu-extruded-sm)] md:hidden"
